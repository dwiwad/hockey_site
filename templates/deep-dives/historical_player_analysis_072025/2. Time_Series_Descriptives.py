...
career_df = (
    roster.groupby('id')['season']
    .agg(first_season='min', last_season='max')
    .reset_index()
)

career_df['first_year'] = career_df['first_season'].astype(str).str[:4].astype(int)
career_df['last_year'] = career_df['last_season'].astype(str).str[:4].astype(int)

career_df['career_length'] = career_df['last_year'] - career_df['first_year'] + 1

latest_season = roster['season'].max()
latest_year = int(str(latest_season)[:4])
career_df['is_active'] = career_df['last_year'] == latest_year

career_df['season_label'] = career_df['first_year'].apply(split_and_hyphenate)

# Compute average career length by debut season
avg_career = (
    career_df.groupby('season_label')['career_length']
    .mean()
    .reset_index()
)
season_labels = avg_career['season_label'].unique()
season_to_index = {label: i for i, label in enumerate(season_labels)}
avg_career['x_pos'] = avg_career['season_label'].map(season_to_index)
...
fig.text(
    0.9,
    0.01,
    "Data: Career length by first season with LOESS-smoothed trend",
    fontsize=10,
    style='italic',
    ha='right'
)
...
ax.legend().remove()
save_figure("nhl_career_length_by_first_season.png")
plt.show()
