"""
This parses hits from the pbp data
...
def _is_hit(play: dict) -> bool:
    k = (
        (play.get("typeDescKey") or "")
        or (play.get("typeCode") or "")
        or (play.get("eventType") or "")
    )
    k = str(k).lower()
    return k == "hit" or k == "503"
...
# Count SOGs by team
def hits_by_team(pbp: dict) -> dict:
